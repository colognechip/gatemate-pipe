...
// Receiver side signals
   ...
   wire       s_link_detected;          // Link number detected
   wire       s_lane_detected;          // Lane number detected
...
//-------------------------------------------------------------------------------------------------------------------------------
// FSM states
   wire [4:0] s_fsm_state;
//-------------------------------------------------------------------------------------------------------------------------------
// Sending triggers and signals
   wire       s_send_IDLE_trigger; // Trigger to send IDLE
   wire       s_send_OS_trigger;   // Trigger to send OS
   wire       s_send_data_trigger; // Trigger to send data
...
//-------------------------------------------------------------------------------------------------------------------------------
// Assignments
   wire idle_state = s_fsm_state == 5'b01001 || s_fsm_state == 5'b10101; // CONFIG_IDLE or RECOVERY.IDLE
   ...
   assign s_rx_rst  = ((s_fsm_state == 5'b10010) ? s_link_detected_rst :
                       (s_fsm_state == 5'b10011) ? s_lane_detected_rst : s_rx_flag_rst) || ~i_RxValid;

   // L0 enabled in L0 state
   //assign L0_enabled   = s_fsm_state == 5'b01010 ? 1'b1 : 1'b0;
   assign L0_enabled   = s_send_data_trigger;
   // Invert Rx polarity
   assign o_RxPolarity = inversion_detected;
//-------------------------------------------------------------------------------------------------------------------------------
   // Count number of received Ordered Sets/IDLE data at each state
   assign rx_max_count = s_fsm_state == 5'b00010 ? 4'b1000 :
                         s_fsm_state == 5'b00011 ? 4'b1000 :
                         s_fsm_state == 5'b00100 ? 4'b0010 :
                         s_fsm_state == 5'b00101 ? 4'b0010 :
                         s_fsm_state == 5'b00110 ? 4'b0010 :
                         s_fsm_state == 5'b00111 ? 4'b0010 :
                         s_fsm_state == 5'b01000 ? 4'b1000 :
                         s_fsm_state == 5'b01001 ? 4'b1000 :
                         s_fsm_state == 5'b10010 ? 4'b0001 :
   ...
   assign TS1_pattern_en = s_fsm_state == 5'b00010 ? 1'b1 :
                           ...
                           s_fsm_state == 5'b00100 ? 1'b1 :
                           s_fsm_state == 5'b00101 ? 1'b1 :
                           s_fsm_state == 5'b00110 ? 1'b1 :
                           s_fsm_state == 5'b00111 ? 1'b1 :
                           s_fsm_state == 5'b01000 ? 1'b0 :
                           s_fsm_state == 5'b01001 ? 1'b0 :
                           s_fsm_state == 5'b10010 ? 1'b1 :
   ...
   assign TS2_pattern_en = s_fsm_state == 5'b00010 ? 1'b1 :
                           ...
                           s_fsm_state == 5'b00100 ? 1'b0 :
                           s_fsm_state == 5'b00101 ? 1'b0 :
                           s_fsm_state == 5'b00110 ? 1'b0 :
                           s_fsm_state == 5'b00111 ? 1'b0 :
                           s_fsm_state == 5'b01000 ? 1'b1 :
                           s_fsm_state == 5'b01001 ? 1'b0 :
                           s_fsm_state == 5'b10010 ? 1'b0 :
                           s_fsm_state == 5'b10011 ? 1'b0 :
                           s_fsm_state == 5'b01011 ? 1'b1 :
                           s_fsm_state == 5'b10100 ? 1'b1 :
                           s_fsm_state == 5'b10101 ? 1'b0 : 1'b0;
   // Count number of transmitted Ordered Sets at each state
   assign tx_max_count   = s_fsm_state == 5'b00011 ? 5'b10000 :
                           s_fsm_state == 5'b01000 ? 5'b10000 :
                           s_fsm_state == 5'b01001 ? 5'b10000 :
                           s_fsm_state == 5'b10100 ? 5'b10000 : 5'b11111;
   // Type of Ordered Set to be sent at each state/ 0:TS1, 1:TS2
   assign OS_type        = s_fsm_state == 5'b00010 ? 1'b0 :
                           s_fsm_state == 5'b00011 ? 1'b1 :
                           s_fsm_state == 5'b00100 ? 1'b0 :
                           s_fsm_state == 5'b00101 ? 1'b0 :
                           s_fsm_state == 5'b00110 ? 1'b0 :
   ...
   assign clk_max_count = s_fsm_state == 5'b00000 ? 12*1000000 / CYCLE_TIME : //12ms
                          ...
                          s_fsm_state == 5'b00101 ?  2*1000000 / CYCLE_TIME : //2ms
                          s_fsm_state == 5'b00110 ?  2*1000000 / CYCLE_TIME : //2ms
                          s_fsm_state == 5'b01000 ?  2*1000000 / CYCLE_TIME : //2ms
...
// Timeout enable and timeout signal
   ...
   assign s_timeout_OS_en  = (( s_fsm_state == 5'b00101 || s_fsm_state == 5'b00110 ) || s_fsm_state == 5'b00111) ? 1'b1 : 1'b0;
...
// ----- LTSSM -----
   ccfpga_LTSSM_fsm #(
      ...
      .s_link_detected              ( s_link_detected  ),              // Link Detected
      .s_lane_detected              ( s_lane_detected  ),              // Lane Detected
      ...
      .s_send_OS_trigger            ( s_send_OS_trigger   ),           // Trigger the sending of OS
      .s_send_IDLE_trigger          ( s_send_IDLE_trigger ),           // Trigger the sending of IDLE
      .s_send_data_trigger          ( s_send_data_trigger ),           // Trigger the sending of data
      .s_fsm_state                  ( s_fsm_state         ),           // fsm status
...
// ----- Receiver of MAC layer -----
   ccfpga_rx_MAC #(
      ...
      .rec_cfg_reset_flag    ( s_recovery_cfg_rx_flag_rst  ),
      .rec_idle_reset_flag   ( s_recovery_idle_rx_flag_rst ),
      ...
      .link_detected         ( s_link_detected             ),
      .lane_detected         ( s_lane_detected             ),
...
// ----- Transmitter of MAC layer -----
   ccfpga_tx_MAC #(
      ...
      .send_OS_trigger              ( s_send_OS_trigger            ),
      .send_IDLE_trigger            ( s_send_IDLE_trigger          ),
      .OS_type                      ( OS_type                      ),
...
// ----- Descrambler -----
   ...
   assign rx_char_is_training_sequence = (s_fsm_state == 5'b01010 | s_fsm_state == 5'b01001 | s_fsm_state == 5'b10110) ?
